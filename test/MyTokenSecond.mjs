...
describe("MyTokenSecond", function () {
    var MyToken;
    var myToken;
    var owner;
    var addr1;
    ...
    beforeEach(async function () {
        MyToken = await hre.ethers.getContractFactory("MyTokenSecond");
        [owner, addr1] = await ethers.getSigners();
        myToken = await MyToken.deploy(10000, 100);
    });

    it("Должен проверить имя токена", async function () {
        expect(await myToken.name()).to.equal("MyTokenSecond");
    });

    it("Должен проверить символ токена", async function () {
        expect(await myToken.symbol()).to.equal("MTS");
    });
